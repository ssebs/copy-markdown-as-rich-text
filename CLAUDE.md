...
- **`src/extension.ts`** — registers the single command `markdown.copyAsRichText`. Reads the active selection (or whole document if empty), pulls config (`copyMarkdownRichText.includeStyles`, `copyMarkdownRichText.highlightCode`), calls `renderMarkdown`, then `writeHtmlToClipboard`. On native clipboard failure it falls back to `vscode.env.clipboard.writeText(html)` and warns the user. The command is also surfaced via an `editor/context` menu entry and the `Ctrl+Shift+C` / `Cmd+Shift+C` keybinding (both declared in `package.json` and gated on `editorTextFocus && editorLangId == markdown`).
- **`src/render.ts`** — pure `renderMarkdown(md, opts)` wrapping `markdown-it` + `highlight.js`. Returns a complete `<!DOCTYPE html>` document with optional inline `<style>` (layout/typography only, no fg/bg colors so the destination's theme wins). Code/`pre` use the generic `monospace` family (doubled to dodge the legacy 13px shrink) so the destination app's configured monospace font wins.
